...
namespace Responsi2629
{
    class Program
    {
        ...
        static void HapusProduk()
        {
            ...
            Produk hps = daftarProduk.SingleOrDefault(f => f.Kode == kode);
            if (daftarProduk.Contains(hps) == true)
            {
                daftarProduk.Remove(hps);
        ...
        static void TampilProduk()
        {
            ...
            foreach (Produk pdc in daftarProduk)
            {
                Console.WriteLine("{0}. {1}, {2}, {3}, {4}", no, pdc.Kode, pdc.Nama, pdc.Beli, pdc.Jual);
                no++;
            }
